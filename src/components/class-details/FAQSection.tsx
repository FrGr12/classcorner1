...
const FAQSection = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('course_faqs').select('*').eq('course_id', courseId).order('display_order');
  ...
  return <section className="glass-panel rounded-xl p-8">
      ...
      {faqs && faqs.length > 0 ? <Accordion type="single" collapsible className="space-y-4">
          {faqs.map(faq => <AccordionItem key={faq.id} value={`faq-${faq.id}`} className="border rounded-lg px-4">
          ...
          <Button variant="outline" onClick={handleAskQuestion} disabled={isSubmitting}>
            Ask a Question
          </Button>
        </div>}
    </section>;
