...
const Features = () => {
  ...
  const handleTeachClick = () => {
    navigate('/auth', {
      state: {
        returnTo: '/dashboard/create-class'
      }
    });
  };
  return <section className="py-24">
      <div className="container-padding">
        ...
        {features.map((feature, index) => <motion.div key={feature.title} className={`flex flex-col md:flex-row gap-12 items-center mb-24 ${index % 2 === 1 ? 'md:flex-row-reverse' : ''}`} initial={{
        opacity: 0,
        y: 20
      }} whileInView={{
        opacity: 1,
        ...
        delay: 0.2
            ...
            <div className="flex-1">
              <h3 className="font-sans mb-4 text-xl font-semibold text-left">{feature.title}</h3>
              <p className="text-neutral-600 mb-6">{feature.description}</p>
              <ul className="space-y-4">
                {feature.points.map(point => <motion.li key={point} className="flex items-center gap-3" initial={{
              opacity: 0,
              x: -20
            }} whileInView={{
              opacity: 1,
              ...
              <div className="mt-8 flex gap-4">
                <button onClick={() => index === 0 ? navigate('/browse') : handleTeachClick()} className="button-primary">
                  {index === 0 ? "Find your next class" : "Start Teaching"}
                </button>
                <button onClick={() => navigate('/community/resource/beginner-guides')} className="px-6 py-3 text-neutral-600 hover:text-accent-purple transition-colors">
                  Learn More
                </button>
              </div>
            </div>
          ...
          delay: 0.3
              ...
              <Card className="glass-panel p-6 rounded-2xl overflow-hidden">
                <img src={feature.image} alt={feature.title} className="w-full h-auto rounded-lg object-cover aspect-video" loading="lazy" />
              </Card>
            </motion.div>
          </motion.div>)}
      </div>
