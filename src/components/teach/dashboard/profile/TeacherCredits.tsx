...
export function TeacherCredits() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        ...
        error
      } = await supabase.from('teacher_premium_features').select('boost_credits').eq('teacher_id', user.id).maybeSingle();
      if (error) throw error;
      if (!teacherFeatures) {
        const {
          data: newFeatures,
          error: insertError
        } = await supabase.from('teacher_premium_features').insert({
          teacher_id: user.id,
          boost_credits: 3,
          is_active: true
        }).select('boost_credits').single();
  ...
  const handleBuyCredits = async (amount: number) => {
    try {
      ...
      const {
        error
      } = await supabase.from('boost_credit_purchases').insert({
        amount,
        price,
  ...
  return <Card className="rounded-none sm:rounded-md border-x-0 sm:border-x">
      <CardHeader className="p-3 sm:p-6">
        ...
        <CardDescription className="sm:text-sm text-xs font-normal text-left">
      ...
      <CardContent className="p-3 sm:p-6 pt-0 sm:pt-0 space-y-3 sm:space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-1.5 sm:gap-4">
            {Object.entries(CREDIT_PRICES).map(([amount, price]) => <Button key={amount} variant="outline" onClick={() => handleBuyCredits(parseInt(amount))} disabled={isLoading} className="h-auto py-1.5 sm:py-4 text-[9px] sm:text-sm">
