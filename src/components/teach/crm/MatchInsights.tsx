  ...
  const fetchInsights = async () => {
    try {
      ...
      const formattedInsights = coursesData.map(course => ({
        ...
        booking_rate: course.course_matches?.length ? 
          (course.bookings?.length / course.course_matches?.length) * 100 : 0,
          ...
          <Table>
            ...
            <TableBody>
              {insights.map((insight) => (
                <TableRow key={insight.course_id}>
                  ...
                  <TableCell>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => handleNotifyMatches(insight.course_id)}
                    >
                      Notify Matches
                    </Button>
                  </TableCell>
