...
const ContactsTable = ({ contacts, isLoading }: ContactsTableProps) => {
  ...
  if (isLoading) {
    return <div className="p-2 text-center text-xs sm:text-sm sm:p-4">Loading contacts...</div>;
  }
      ...
      <div className="space-y-2 sm:space-y-4">
        <div className="mb-2 sm:mb-4">
          <select
            ...
            className="w-full border rounded-md px-2 py-1 sm:px-3 sm:py-2 text-xs sm:text-sm h-7 sm:h-10"
          >
        ...
        <div className="relative overflow-x-auto -mx-2 sm:mx-0">
          <Table className="w-[530px] sm:w-full table-fixed">
            <TableHeader>
              <TableRow className="border-b-0 sm:border-b">
                <TableHead 
                  className="cursor-pointer text-[9px] sm:text-sm py-0.5 sm:py-3 px-0.5 sm:px-4 w-[25%]"
                ...
                <TableHead 
                  className="cursor-pointer text-[9px] sm:text-sm py-0.5 sm:py-3 px-0.5 sm:px-4 w-[25%] hidden sm:table-cell"
                ...
                <TableHead className="text-[9px] sm:text-sm py-0.5 sm:py-3 px-0.5 sm:px-4 w-[15%] hidden sm:table-cell">Phone</TableHead>
                <TableHead className="text-[9px] sm:text-sm py-0.5 sm:py-3 px-0.5 sm:px-4 w-[20%]">Tags</TableHead>
                <TableHead 
                  className="cursor-pointer text-[9px] sm:text-sm py-0.5 sm:py-3 px-0.5 sm:px-4 w-[20%]"
                ...
                <TableHead 
                  className="cursor-pointer text-[9px] sm:text-sm py-0.5 sm:py-3 px-0.5 sm:px-4 w-[10%]"
                ...
                <TableHead className="text-[9px] sm:text-sm py-0.5 sm:py-3 px-0.5 sm:px-4 w-[25%]"></TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredAndSortedContacts.map((contact) => (
                <TableRow key={contact.id} className="border-b-0 sm:border-b">
                  <TableCell className="font-medium text-[9px] sm:text-sm py-0.5 sm:py-4 px-0.5 sm:px-4 truncate">
                    {contact.first_name} {contact.last_name}
                  </TableCell>
                  <TableCell className="text-[9px] sm:text-sm py-0.5 sm:py-4 px-0.5 sm:px-4 truncate hidden sm:table-cell">
                    {contact.email}
                  </TableCell>
                  <TableCell className="text-[9px] sm:text-sm py-0.5 sm:py-4 px-0.5 sm:px-4 hidden sm:table-cell">{contact.phone || '-'}</TableCell>
                  <TableCell className="text-[9px] sm:text-sm py-0.5 sm:py-4 px-0.5 sm:px-4">
                    <div className="flex gap-0.5 sm:gap-2 flex-wrap">
                      {contact.tags.slice(0, 1).map((tag) => (
                        <Badge
                          key={tag}
                          className={`${getTagColor(tag)} text-[7px] sm:text-xs px-1 sm:px-3 py-0 sm:py-1 truncate max-w-[50px] sm:max-w-none`}
                        >
                          {tag}
                        </Badge>
                      ))}
                      {contact.tags.length > 1 && (
                        <Badge className="bg-gray-100 text-[7px] sm:text-xs px-0.5 sm:px-2 py-0 sm:py-0.5">
                          +{contact.tags.length - 1}
                        </Badge>
                      )}
                    </div>
                  </TableCell>
                  <TableCell className="text-[9px] sm:text-sm py-0.5 sm:py-4 px-0.5 sm:px-4 whitespace-nowrap">
                    {format(new Date(contact.last_interaction), 'MM/dd/yy')}
                  </TableCell>
                  <TableCell className="text-[9px] sm:text-sm py-0.5 sm:py-4 px-0.5 sm:px-4">{contact.total_bookings}</TableCell>
                  <TableCell className="text-[9px] sm:text-sm py-0.5 sm:py-4 px-0.5 sm:px-4">
                    <div className="flex gap-1 sm:gap-4 justify-end">
                      <Button 
                        variant="secondary"
                        size="icon"
