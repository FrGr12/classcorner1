  ...
  return (
    ...
    >
      <div>
        ...
        {session.is_recurring && session.recurring_weeks && session.recurrence_end_date && (
          <p className="text-xs text-muted-foreground mt-1">
            Repeats for {session.recurring_weeks} week{session.recurring_weeks > 1 ? 's' : ''} 
            until {typeof session.recurrence_end_date === 'string' 
              ? format(new Date(session.recurrence_end_date), "PP") 
              : format(session.recurrence_end_date as Date, "PP")}
          </p>
        )}
      </div>
      <Button
