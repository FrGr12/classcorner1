    ...
    <div className="space-y-1">
        ...
        <div className="relative">
          ...
          {isOpen && (
            <div className="absolute top-full left-0 w-full mt-2 py-1 bg-white border rounded-md shadow-lg z-50">
              <button
                className="w-full px-3 py-2 text-left text-sm font-medium hover:bg-accent/50 transition-colors"
                onClick={() => {
                  onAllPostsClick();
                  setIsOpen(false);
                }}
              ...
              {displayedTopics?.map(topicItem => (
                <button
                  key={topicItem.name}
                  className="w-full px-3 py-2 text-left text-sm font-medium hover:bg-accent/50 transition-colors flex justify-between"
                  onClick={() => {
      ...
      <nav className="hidden lg:block space-y-1">
        <button 
          ...
          className={`w-full flex items-center justify-between px-3 py-2 text-sm font-medium hover:bg-accent rounded-lg transition-colors ${!topic && category === 'all' ? 'bg-accent text-accent-purple' : 'text-neutral-700'}`}
        >
          <span>All Posts</span>
        </button>
        ...
        {displayedTopics?.map(topicItem => (
          <button 
            ...
            className={`w-full flex items-center justify-between px-3 py-2 text-sm font-medium hover:bg-accent rounded-lg transition-colors ${topic === topicItem.name.toLowerCase().replace(/ /g, '-') ? 'bg-accent text-accent-purple' : 'text-neutral-700'}`}
          >
            <span>{topicItem.name}</span>
            <span className="text-muted-foreground text-xs">
              {topicItem.count}
