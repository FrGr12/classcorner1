...
const TeacherWaitlist = () => {
  ...
  return <div className="space-y-6 text-left">
      {Object.entries(entriesByCourse).map(([courseId, entries]) => <Card key={courseId}>
          <CardHeader>
            <div className="flex justify-between items-start">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-end gap-2">
                  ...
                  <Switch checked={entries[0].course.auto_promote_from_waitlist} onCheckedChange={() => toggleAutoPromote(Number(courseId), entries[0].course.auto_promote_from_waitlist)} />
                </div>
                <div className="flex items-center justify-end gap-2">
                  <span className="text-sm text-muted-foreground">Auto-notify</span>
                  <Switch checked={entries[0].course.auto_send_waitlist_notification} onCheckedChange={() => toggleAutoNotify(Number(courseId), entries[0].course.auto_send_waitlist_notification)} />
                </div>
              </div>
            </div>
          </CardHeader>
            ...
            <Table>
              ...
              <TableBody>
                {entries.map(entry => <TableRow key={entry.id}>
                    ...
                    <TableCell>
                      {entry.profile ? `${entry.profile.first_name} ${entry.profile.last_name}` : "Anonymous User"}
                    </TableCell>
                    <TableCell>
                      {new Date(entry.created_at).toLocaleDateString()}
                    </TableCell>
                    <TableCell>
                      {entry.notification_sent_at ? new Date(entry.notification_sent_at).toLocaleDateString() : 'Never'}
                    </TableCell>
                    <TableCell>
                      <Button size="sm" variant="outline" onClick={() => handleNotify(entry)} disabled={entry.notification_sent_at ? new Date().getTime() - new Date(entry.notification_sent_at).getTime() < 24 * 60 * 60 * 1000 : false}>
                        Notify
                      </Button>
                    </TableCell>
                  </TableRow>)}
