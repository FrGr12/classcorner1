        ...
        <div className="border-b bg-card">
          <div className="container mx-auto px-4 py-[52px]">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="font-bold mb-2 text-xl">Groups</h1>
          ...
          <Tabs defaultValue="discover" className="space-y-6">
            ...
            <TabsContent value="discover" className="space-y-6">
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {DUMMY_GROUPS.map(group => <GroupCard key={group.id} id={group.id} name={group.name} description={group.description} memberCount={group.member_count} type={group.type} topic={group.topic} region={group.region} onJoin={handleJoinGroup} />)}
              </div>
            </TabsContent>

            <TabsContent value="joined">
              {isLoadingJoined ? <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                  {[...Array(3)].map((_, i) => <div key={i} className="h-48 bg-muted rounded-lg animate-pulse" />)}
                </div> : joinedGroups?.length ? <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                  {joinedGroups.map((group: any) => <GroupCard key={group.id} id={group.id} name={group.name} description={group.description} memberCount={group.member_count} type={group.type} topic={group.topic} region={group.region} onJoin={() => navigate(`/community/groups/${group.id}`)} />)}
